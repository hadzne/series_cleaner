...
all_files_se = [x for x in all_files if re.match(regex, x)]
...
for tx in txs:
    ...
    tx_se = re.findall(regex, tx)[0]
    multimedia_se =  [re.findall(regex, x)[0] for x in all_files_se if x.split('.')[-1] in multimedia_extension]
    txt_se =  [re.findall(regex, x)[0] for x in all_files_se if x.split('.')[-1] == 'txt']
